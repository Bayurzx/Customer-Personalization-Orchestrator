...
def calculate_power_proportion(p1: float, p2: float, n: int, alpha: float = 0.05) -> float:
    """
...
def create_power_visualizations(power_df: pd.DataFrame, output_dir: Path):
    ...
    for i, metric in enumerate(metrics):
        ...
        for effect_size in metric_data["effect_size"].unique():
            ...
            axes[i].plot(
                data["sample_size_per_arm"],
                data["power"],
                marker="o",
                label=f"{effect_size:.0%} lift",
            )
    ...
    for metric in metrics:
        ...
        pivot_data = metric_data.pivot(
            index="effect_size", columns="sample_size_per_arm", values="power"
        )

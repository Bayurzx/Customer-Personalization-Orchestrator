...
def run_manual_tests():
    ...
    for tone in tones:
        ...
        if result['success']:
            ...
            print(f"   Subject length: {val.get('subject_length', 'N/A')} chars ({'✅' if val.get('subject_valid') else '❌'})")
            print(f"   Body word count: {val.get('body_word_count', 'N/A')} words ({'✅' if val.get('body_valid') else '❌'})")
            print(f"   Citations: {val.get('citation_count', 'N/A')} found ({'✅' if val.get('citations_valid') else '❌'})")

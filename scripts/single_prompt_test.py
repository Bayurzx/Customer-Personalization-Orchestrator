...
def run_single_test():
    ...
    try:
        ...
        if validation.get('subject'):
            ...
            print(f"   Subject length: {validation['subject_length']} chars ({'✅' if validation.get('subject_valid') else '❌'})")
        
        if validation.get('body_word_count'):
            print(f"   Body word count: {validation['body_word_count']} words ({'✅' if validation.get('body_valid') else '❌'})")
        
        if 'citation_count' in validation:
            print(f"   Citations: {validation['citation_count']} found ({'✅' if validation.get('citations_valid') else '❌'})")

...
def run_safety_screening(
    variants: List[Dict[str, Any]], safety_agent: SafetyAgent
) -> List[Dict[str, Any]]:
    """
    ...
    for i, variant in enumerate(variants, 1):
        try:
            logger.info(
                f"Screening variant {i}/{len(variants)}: {variant.get('variant_id', 'UNKNOWN')}"
            )
            ...
            if result["status"] == "block":
                logger.warning(f"   Blocked categories: {result['blocked_categories']}")
                logger.warning(f"   Reason: {result['block_reason']}")
...
def calculate_safety_metrics(safety_results: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    # Category breakdown
    category_blocks = {"hate": 0, "violence": 0, "self_harm": 0, "sexual": 0, "api_error": 0}
    ...
    severity_distribution = {"safe_0": 0, "low_2": 0, "medium_4": 0, "high_6": 0}
...
def review_blocked_variants(safety_results: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for result in blocked_variants:
        ...
        if len(result.get("blocked_categories", [])) > 1:
            review_info["review_notes"].append(
                "MULTIPLE CATEGORIES: Multiple policy violations detected"
            )
...
def verify_audit_log_completeness(safety_agent: SafetyAgent, expected_count: int) -> Dict[str, Any]:
    """
...
def print_summary_results(
    ...
    print(
        f"   Passed: {safety_metrics['passed_variants']} ({safety_metrics['pass_rate_percent']:.1f}%)"
    )
    print(
        f"   Blocked: {safety_metrics['blocked_variants']} ({safety_metrics['block_rate_percent']:.1f}%)"
    )
    print(
        f"   Errors: {safety_metrics['error_variants']} ({safety_metrics['error_rate_percent']:.1f}%)"
    )
    ...
    for segment, stats in safety_metrics["segment_breakdown"].items():
        print(
            f"   {segment}: {stats['passed']}/{stats['total']} passed ({stats['pass_rate']:.1f}%)"
        )
    ...
    print(f"\n📋 AUDIT LOG VERIFICATION:")
    print(
        f"   Complete: {audit_status} ({audit_verification['actual_entries']}/{audit_verification['expected_entries']} entries)"
    )

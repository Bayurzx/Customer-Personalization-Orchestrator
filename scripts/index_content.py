...
def validate_documents(documents: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for i, doc in enumerate(documents):
        try:
            ...
            if len(content) < 50:
                logger.warning(
                    f"⚠️ Document {doc_id}: content too short ({len(content)} chars), skipping"
                )
                continue
    ...
    logger.info(
        f"✅ Validated {len(valid_documents)} documents (filtered out {len(documents) - len(valid_documents)})"
    )
...
def index_content_pipeline(
    ...
    try:
        ...
        indexing_result = index_documents(
            documents=valid_documents, index_name=index_name, batch_size=batch_size
        )
...
Examples:
  ...
  python scripts/index_content.py --batch-size 50 --force
    ...
    parser.add_argument(
        "--force", action="store_true", help="Recreate the index if it already exists"
    )

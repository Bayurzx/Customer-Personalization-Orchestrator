...
class ExperimentPipeline:
    ...
    def run_full_experiment(self, customer_data_path: str = "data/raw/customers.csv") -> Dict[str, Any]:
        """
    ...
    def _run_content_retrieval(self, segments_df: pd.DataFrame) -> Dict[str, List[Dict]]:
        ...
        try:
            ...
            logger.info(f"Retrieved {total_retrieved} total content pieces across {len(unique_segments)} segments")
            
            self.results['content_retrieval'] = {
                'segments_processed': len(unique_segments),
                'total_content_retrieved': total_retrieved,
                'avg_content_per_segment': total_retrieved / len(unique_segments) if unique_segments.size > 0 else 0
            }
    ...
    def _run_message_generation(self, segments_df: pd.DataFrame, retrieved_content: Dict[str, List[Dict]]) -> List[Dict]:
        ...
        try:
            ...
            with tqdm(unique_segments, desc="Generating variants") as pbar:
                for segment_name in pbar:
                    ...
                    if not content:
                        logger.warning(f"No content available for {segment_name}, skipping generation")
                        continue
                    ...
                    try:
                        ...
                        for variant in variants:
                            ...
                            variant['variant_id'] = f"VAR_{segment_name}_{variant.get('tone', 'unknown')}_{len(all_variants)}"
            ...
            logger.info(f"Generated {len(all_variants)} total variants across {len(unique_segments)} segments")
            
            self.results['message_generation'] = {
                'segments_processed': len(unique_segments),
                'total_variants_generated': len(all_variants),
                'avg_variants_per_segment': len(all_variants) / len(unique_segments) if unique_segments.size > 0 else 0
            }
    ...
    def _run_safety_screening(self, variants: List[Dict]) -> List[Dict]:
        ...
        try:
            ...
            with tqdm(variants, desc="Safety screening") as pbar:
                for variant in pbar:
                    pbar.set_description(f"Screening variant {variant.get('variant_id', 'unknown')}")
                    ...
                    try:
                        ...
                        logger.warning(f"Safety check failed for variant {variant.get('variant_id')}: {e}")
                        # Fail closed - treat as blocked
                        blocked_variants.append({
                            'variant': variant,
                            'safety_result': {'status': 'block', 'block_reason': f'Safety check error: {e}'}
                        })
            ...
            logger.info(f"Safety screening: {len(safe_variants)}/{len(variants)} variants passed ({safety_summary['pass_rate']:.1%} pass rate)")
    ...
    def _run_experiment(self, customers_df: pd.DataFrame, safe_variants: List[Dict]) -> Dict[str, Any]:
        ...
        try:
            ...
            assignments = exp_agent.assign_customers_to_arms(
                customers_with_segments.to_dict(orient='records'),
                experiment_design
            )
            ...
            logger.info(f"Experiment completed: {len(assignments)} customers assigned across {len(experiment_design.get('arms', []))} arms")
    ...
    def _generate_final_summary(self, experiment_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            serializable_results = json.loads(json.dumps(self.results, default=self._json_serializer))
            ...
            summary = {
                ...
                'quality_metrics': {
                    'segmentation_success': self.results.get('segmentation', {}).get('segments_created', 0) >= 3,
                    'content_retrieval_success': self.results.get('content_retrieval', {}).get('total_content_retrieved', 0) > 0,
                    'generation_success': self.results.get('message_generation', {}).get('total_variants_generated', 0) > 0,
                    'safety_pass_rate': float(self.results.get('safety_screening', {}).get('pass_rate', 0)),
    ...
    def _print_summary(self, summary: Dict[str, Any]):
        ...
        print(f"   • Customers Segmented: {pipeline.get('segmentation', {}).get('total_customers', 0)}")
        print(f"   • Segments Created: {pipeline.get('segmentation', {}).get('segments_created', 0)}")
        print(f"   • Content Retrieved: {pipeline.get('content_retrieval', {}).get('total_content_retrieved', 0)}")
        print(f"   • Variants Generated: {pipeline.get('message_generation', {}).get('total_variants_generated', 0)}")
        print(f"   • Safety Pass Rate: {pipeline.get('safety_screening', {}).get('pass_rate', 0):.1%}")
        ...
        print(f"   • Statistical Significance: {'✅ Yes' if exp_summary['statistical_significance'] else '❌ No'}")

...
def validate_engagement_simulation(engagement_data, assignments, config):
    ...
    assert 0.15 <= overall_open_rate <= 0.40, f"Open rate {overall_open_rate:.1%} outside realistic range (15-40%)"
    assert 0.02 <= overall_click_rate <= 0.15, f"Click rate {overall_click_rate:.1%} outside realistic range (2-15%)"
    ...
    for arm, rates in arm_rates.items():
        print(f"   {arm:12}: Open={rates['open_rate']:.1%}, Click={rates['click_rate']:.1%}, Convert={rates['conversion_rate']:.1%} (n={rates['sample_size']})")
    ...
    required_fields = ['customer_id', 'segment', 'experiment_arm', 'variant_id', 
                      'opened', 'clicked', 'converted', 'engagement_at', 'engagement_source']
    ...
    for i, record in enumerate(engagement_data[:5]):  # Check first 5 records
        ...
        # Check data types
        assert isinstance(record['opened'], bool), f"'opened' should be bool, got {type(record['opened'])}"
        assert isinstance(record['clicked'], bool), f"'clicked' should be bool, got {type(record['clicked'])}"
        assert isinstance(record['converted'], bool), f"'converted' should be bool, got {type(record['converted'])}"
...
def test_reproducibility(assignments, config):
    ...
    for e1, e2 in zip(engagement1, engagement2):
        if (e1['opened'] == e2['opened'] and 
            e1['clicked'] == e2['clicked'] and 
...
def main():
    ...
    if has_historical:
        ...
        print(f"   Baseline rates: Open={baseline_rates.get('open_rate', 0):.1%}, Click={baseline_rates.get('click_rate', 0):.1%}")
    ...
    try:
        ...
        if control_open_rate > 0:
            assert treatment_open_rate > control_open_rate, f"Treatment open rate ({treatment_open_rate:.1%}) not greater than control ({control_open_rate:.1%})"
    ...
    try:
        ...
        summary = {
            ...
            'overall_rates': {
                ...
                'click_rate': sum(1 for e in engagement_data if e['clicked']) / len(engagement_data),
                'conversion_rate': sum(1 for e in engagement_data if e['converted']) / len(engagement_data)
            },
            'arm_breakdown': {},

...
def check_azure_cli():
    ...
    try:
        ...
        if result.returncode != 0:
            print(
                "❌ Azure CLI not found. Please install: https://learn.microsoft.com/en-us/cli/azure/install-azure-cli"
            )
            return False
...
def create_resource_group(name: str, location: str) -> bool:
    ...
    try:
        ...
        result = run_command(["az", "group", "show", "--name", name], check=False)
        ...
        run_command(["az", "group", "create", "--name", name, "--location", location])
...
def create_openai_resource(name: str, resource_group: str, location: str) -> Optional[Dict]:
    ...
    try:
        ...
        return {"endpoint": endpoint, "api_key": api_key, "resource_name": name}
...
def deploy_openai_model(
    resource_name: str, resource_group: str, deployment_name: str = "gpt-4o-mini"
) -> bool:
...
def create_search_service(name: str, resource_group: str, location: str) -> Optional[Dict]:
    ...
    try:
        ...
        # Check if already exists
        result = run_command(
            ["az", "search", "service", "show", "--name", name, "--resource-group", resource_group],
            check=False,
        )
...
def create_ml_workspace(name: str, resource_group: str, location: str) -> Optional[Dict]:
    ...
    try:
        ...
        # Check if already exists
        result = run_command(
            ["az", "ml", "workspace", "show", "--name", name, "--resource-group", resource_group],
            check=False,
        )
...
def update_env_file(resources: Dict) -> bool:
    ...
    try:
        ...
        if "ml" in resources:
            updates.update({"AZURE_ML_WORKSPACE_NAME": resources["ml"]["workspace_name"]})
...
def main():
    ...
    resources = {"subscription_id": "", "resource_group": resource_group, "location": location}

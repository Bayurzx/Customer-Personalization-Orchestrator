    ...
    def generate_completion(
        ...
        try:
            ...
            response = self.client.responses.create(
                model=self.deployment_name, input=full_prompt, max_output_tokens=max_tokens
            )
...
def generate_completion(
    prompt: str, system_message: str = "You are a marketing copywriter.", max_tokens: int = 400
) -> Dict[str, Any]:
    """
    ...
    response = client.responses.create(
        model=deployment_name, input=full_prompt, max_output_tokens=max_tokens
    )
    ...
    return {
        ...
        "finish_reason": getattr(response, "finish_reason", "completed"),
        "tokens_used": (
            getattr(response.usage, "total_tokens", 0) if hasattr(response, "usage") else 0
        ),
        "input_tokens": (

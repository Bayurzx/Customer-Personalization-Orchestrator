...
class ContentSafetyClient:
    ...
    def _parse_safety_response(self, response, original_text: str) -> Dict[str, Any]:
        ...
        # Alternative parsing for different response formats
            ...
            result['severity_scores']['violence'] = getattr(response.violence_result, 'severity', 0) if hasattr(response, 'violence_result') else 0
            result['severity_scores']['self_harm'] = getattr(response.self_harm_result, 'severity', 0) if hasattr(response, 'self_harm_result') else 0
            result['severity_scores']['sexual'] = getattr(response.sexual_result, 'severity', 0) if hasattr(response, 'sexual_result') else 0

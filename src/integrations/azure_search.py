...
def get_search_client(index_name: Optional[str] = None) -> SearchClient:
    ...
    return SearchClient(
        endpoint=endpoint, index_name=index_name, credential=AzureKeyCredential(query_key)
    )
...
def create_content_index_schema(index_name: str) -> SearchIndex:
    ...
    fields = [
        ...
        SimpleField(
            name="audience", type=SearchFieldDataType.String, filterable=True, facetable=True
        ),
        # Keywords as a simple searchable string for now (we'll join the array)
        SearchableField(
            name="keywords", type=SearchFieldDataType.String, searchable=True, filterable=True
        ),
        # Date field
        SimpleField(
            name="approval_date",
        ...
        SimpleField(name="metadata_author", type=SearchFieldDataType.String, filterable=True),
        SimpleField(name="metadata_version", type=SearchFieldDataType.String, filterable=True),
        SimpleField(name="metadata_last_updated", type=SearchFieldDataType.String, filterable=True),
    ]
...
def index_documents(
    documents: List[Dict[str, Any]], index_name: Optional[str] = None, batch_size: int = 100
) -> Dict[str, int]:
    """

...
class ExperimentationAgent:
    ...
    def _normalize_config(self, config: Dict) -> Dict:
        ...
        sim_config.setdefault(
            "expected_uplift",
            {"mean": 0.15, "std_dev": 0.05, "min_uplift": 0.05, "max_uplift": 0.30},
        )
    ...
    def design_experiment(self, variants: List[Dict], config: Dict) -> Dict:
        ...
        arms = {
            "control": {
                "name": "control",
                "description": config.get("arms", {})
                .get("control", {})
                .get("description", "Generic control message"),
                "variant_type": "generic",
                "message": config.get(
                    "control_message", {"subject": "Control", "body": "Control message"}
                ),
            }
        }
        ...
        for i, tone in enumerate(tones, 1):
            ...
            arms[treatment_key] = {
                ...
                "description": config.get("arms", {})
                .get(treatment_key, {})
                .get("description", f"Treatment {i}"),
                "variant_type": "personalized",
                "tone": tone,
                "variants_by_segment": {
                    segment: variants_by_segment[segment].get(tone) for segment in segments
                },
            }
    ...
    def assign_customers_to_arms(
        self, customers: List[Dict], experiment_design: Dict
    ) -> List[Dict]:
        """
        ...
        for segment, segment_customers in customers_by_segment.items():
            ...
            treatment_size = max(
                1, int(n_customers * allocation_pct["treatment_percent"] / 100 / 3)
            )  # 3 treatment arms
            ...
            for arm_idx, arm_name in enumerate(treatment_arms):
                ...
                for i in range(arm_start, min(arm_end, len(shuffled_customers))):
                    ...
                    variant_id = (
                        variant["variant_id"] if variant else f"{arm_name}_{segment}_fallback"
                    )
    ...
    def _validate_assignment_balance(self, assignments: List[Dict], experiment_design: Dict):
        ...
        tolerance = (
            experiment_design.get("quality_checks", {})
            .get("assignment_balance", {})
            .get("tolerance", 0.05)
        )
        ...
        for arm, count in arm_counts.items():
            ...
            if deviation > (tolerance * 100):
                logger.warning(
                    f"Assignment imbalance detected for {arm}: {actual_pct:.1f}% (target: {target_pct}%)"
                )
            else:
                logger.info(f"Assignment balance OK for {arm}: {actual_pct:.1f}%")

    def simulate_engagement(
        self, assignments: List[Dict], config: Dict, use_historical: bool = False
    ) -> List[Dict]:
        """
        ...
        for assignment in assignments:
            ...
            open_baseline = segment_config.get(
                "expected_baseline_open", baseline_rates["open_rate"]
            )
            click_baseline = segment_config.get(
                "expected_baseline_click", baseline_rates["click_rate"]
            )
            ...
            converted = clicked and (
                np.random.random() < conversion_prob
            )  # Can only convert if clicked
    ...
    def _log_engagement_stats(self, engagement_data: List[Dict]):
        ...
        for arm, stats in stats_by_arm.items():
            ...
            logger.info(
                f"  {arm}: Open={open_rate:.1f}%, Click={click_rate:.1f}%, Convert={conversion_rate:.1f}% (n={total})"
            )
    ...
    def _calculate_arm_metrics(self, arm_data: List[Dict]) -> Dict:
        ...
        return {
            ...
            "counts": {"sent": total, "opened": opened, "clicked": clicked, "converted": converted},
        }
    ...
    def _calculate_statistical_significance(
        self, control_data: Dict, treatment_data: Dict, metric: str
    ) -> Dict:
        """
    ...
    def _calculate_segment_breakdown(self, engagement_data: List[Dict]) -> List[Dict]:
        ...
        for segment, segment_data in data_by_segment.items():
            ...
            control_click_rate = (
                segment_metrics.get("control", {}).get("metrics", {}).get("click_rate", 0)
            )
...
def assign_customers_to_arms(
    customers: List[Dict], variants: List[Dict], config: Dict
) -> List[Dict]:
    """

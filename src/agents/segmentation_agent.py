...
def _segment_by_rules(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, customer in df.iterrows():
        ...
        engagement_score = (customer['historical_open_rate'] + 
                          customer['historical_click_rate']) / 2
        ...
        elif (customer['purchase_frequency'] > 12 and
...
def _generate_cluster_names(df: pd.DataFrame, labels: np.ndarray, features: pd.DataFrame) -> Dict[int, str]:
    """
...
def generate_segment_summary(segments_df: pd.DataFrame) -> Dict:
    ...
    for segment in unique_segments:
        ...
        avg_purchase_freq = segment_data['features'].apply(
            lambda x: x['avg_purchase_frequency']).mean()
        avg_order_value = segment_data['features'].apply(
            lambda x: x['avg_order_value']).mean()
        avg_engagement = segment_data['features'].apply(
            lambda x: x['engagement_score']).mean()
